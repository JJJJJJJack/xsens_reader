...
template <typename X> DMatrix<X> DMatrix<X>::diag(const DVector<X>& v){
	DMatrix<X> mat(v.dim(), v.dim());
...
template <class X> DMatrix<X>& DMatrix<X>::operator+=(const DMatrix<X>& m) {
	if (ncols != m.ncols || nrows != m.nrows) throw DIncompatibleMatrixException();
	if ((*shares)>1) detach();
	int cnt = nrows*ncols;
	X* tmpElems1 = elems;
...
template <class X> DMatrix<X>& DMatrix<X>::operator*=(const X& e) {
	if ((*shares)>1) detach();
	int cnt = nrows*ncols;
	for (int i = 0; i < cnt; ++i)
		elems[i] *= e;
